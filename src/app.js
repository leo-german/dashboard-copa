...
function updateBaseDeDatosSection(selectedYm) {
    ...
    prevData.forEach(item => {
        const monto = item.monto || 0;
        if (item.tipo === 'Ingreso') prevIngresosTotal += monto;
        else if (item.tipo === 'Egreso') prevEgresosTotal += monto;
    });
    ...
    // Agrupar montos por categoría del mes seleccionado
    const catMap = {};
    currentData.forEach(item => {
    ...
    charts.categoriasBase = new Chart(ctxCat, {
        ...
        data: {
            labels: catLabels,
            datasets: [{
                label: 'Monto Neto por Categoría',
                data: catValues,
                backgroundColor: catValues.map(v => v >= 0 ? '#2E7D32' : '#D32F2F'),
                borderRadius: 4
            }]
        },
        options: {
...
function updateFinancialSection() {
    ...
    // Filtrar capital del mes seleccionado de constitución
    const currentCap = rawData.capital_financiero.filter(item => item.fecha_constitucion && item.fecha_constitucion.startsWith(selectedYm));
    ...
    document.getElementById('kpi-cap-total').textContent = formatCurrency(totalCapital);
    document.getElementById('kpi-cap-interes').textContent = formatCurrency(totalIntereses);
    ...
    if (currentCap.length > 0) {
        monedaEl.textContent = currentCap[0].moneda || 'ARS';
    } else {
        monedaEl.textContent = 'ARS';
    }
    ...
    currentCap.forEach(item => {
        ...
        tr.innerHTML = `
            ...
            <td><span style="color: var(--copa-success)">+${formatCurrency(item.interes)}</span></td>
            <td><strong>${formatCurrency(item.monto_final)}</strong></td>
